...
def wavcrawler_data_process(segment, mode, channels, segment_dur, calibrate=False, sample_rate=4000):
...
def generate_single_channel_mfcc(audio, mode, source, segment_dur, sample_rate):
    """
...
def generate_multi_channel_mfcc(audio, mode, source, sample_rate):
    """
        This function processes the filepaths read-in from the csv file into mel log STFTs
...
def full_mel_mfcc_pipeline(data, channels, mode, source, segment_dur, calibrate, sample_rate):

    #signal.signal(signal.SIGSEGV, sig_handler)

    if source == 'wc':
        data, times = wavcrawler_data_process(data, mode, channels, segment_dur, calibrate, sample_rate)

    if mode == 'single':
        if channels==1:
            #spectrogram = generate_spectrogram_mfcc(filepaths)
            #dataset = generate_mel_spectrogram_mfcc(spectrogram)
            dataset = generate_single_channel_mfcc(data, mode, source, segment_dur, sample_rate)
        elif channels==4:
            dataset = generate_multi_channel_mfcc(data, mode, source, sample_rate)
        ...
        while seg:
            SEG_HAPPENED = False
            print("Processing spectrogram", flush=True)
            if channels==1:
            # Perform spectrogram operation in single function
                dataset = dataset.map(lambda x: generate_single_channel_mfcc(x, mode, source, segment_dur, sample_rate), num_parallel_calls=tf.data.experimental.AUTOTUNE)
            elif channels==4:
                dataset = dataset.map(lambda x: generate_multi_channel_mfcc(x, mode, source, sample_rate), num_parallel_calls=tf.data.experimental.AUTOTUNE)
            print("Spectrogram processed", flush=True)
            if SEG_HAPPENED:
                continue
            else:
                seg=False
        ...
        print('Please select either single or batch for data processing mode', flush=True)
