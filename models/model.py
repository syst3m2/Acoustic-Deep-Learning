...
from models.dev_model_bnn import build_model as build_dev_bnn_model
from models.cnn_rect_model import build_model as build_cnn_rect_model
from models.pretrained_models import vgg_pretrained, inception_pretrained, pretrained_model, mobilenet_pretrained
from models.resnet_models import resnet_v1_model, resnet_v2_model
from models.simple_models import simple_model, simple_BNN_VI_model, simple_MC_model
from models.resnet_bnn import build_model as build_resnet_bnn_model
from models.resnet_rect_bnn import build_model as build_resnet_rect_bnn_model
...
model_dict = {"simple":simple_model, "resnet1":resnet_v1_model, "resnet2":resnet_v2_model, "vgg":vgg_pretrained, 
              ...
              'resnet_bnn':build_resnet_bnn_model, 'resnet_rect_bnn':build_resnet_rect_bnn_model}


def build_model(param, input_shape, activation_fcn,ds_size,hparams=None):
    bnn = False
    if param.bnn_type != 'none':
        bnn = True
    
    model_choice = param.model.lower()
    ...
    if model_choice == 'cnn_model' or model_choice == 'cnn_model_hparam':
        return build_cnn_rect_model(param, input_shape, activation_fcn,hparams=hparams)
    elif model_choice == 'dev_bnn_model' or model_choice == 'dev_model_hparam':
        return build_dev_bnn_model(param, input_shape, activation_fcn,ds_size,hparams=hparams)
    elif model_choice == 'resnet_bnn':
        return build_resnet_bnn_model(param, input_shape, activation_fcn,ds_size,hparams=hparams)
    elif model_choice == 'resnet_rect_bnn':
        return build_resnet_bnn_model(param, input_shape, activation_fcn,ds_size,hparams=hparams)
    elif model_choice == 'simple_bnn_vi_model':
        return simple_BNN_VI_model(input_shape, param.num_classes, activation_fcn, param.saved_model_path, param.resnet_depth,ds_size)

    return model_dict[model_choice](input_shape, param.num_classes, activation_fcn, param.saved_model_path, param.resnet_depth)
