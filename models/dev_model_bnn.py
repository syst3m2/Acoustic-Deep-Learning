...
def build_model(param, input_shape, activation_fcn,ds_size,hparams=None):
    ...
    if bnn_type == 'none':
        return dev_model(input_shape, param.num_classes, activation_fcn, False, hparams)
    elif bnn_type == 'dropout':
        return dev_model(input_shape, param.num_classes, activation_fcn, True, hparams,param.mc_dropout_prob,param.dropout_l2)
    elif bnn_type == 'tfp_reparameterization':
        if param.kl_term == -1:
            size = ds_size
        else:
            size = param.kl_term
        return  dev_model_Reparameterization(input_shape, param.num_classes, activation_fcn, size, hparams)
    elif bnn_type == 'tfp_flipout':
        if param.kl_term == -1:
            size = ds_size
        else:
            size = param.kl_term
        return  dev_model_tfp_flipout(input_shape, param.num_classes, activation_fcn, size, hparams)