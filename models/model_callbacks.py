...
def make_callbacks(params):
    ...
    if 'KLreweighing' in params.callbacks:
        checkpoints.append(_make_KLreweighing(params))
...
def _make_KLreweighing(params):
    klreweighing = KLreweighing()
    return klreweighing
...
def _make_checkpoint(params):
    ...
    # Add code, grab max epoch from filenames in directory, add 1 to it if in cont-train mode
...
class KLLossScheduler(tf.keras.callbacks.Callback):
    ...
    def __init__(self, n_silent_epoch=20, n_annealing_epoch=100, verbose=1):
...
class KLreweighing (tf.keras.callbacks.Callback):
    def __init__(self, update_per_batch=True, n_silent_epoch=5, n_annealing_epoch=50,steps_per_epoch=1, verbose=0):
        ...
        super(KLreweighing, self).__init__()
    def on_train_batch_begin(self, batch, logs=None):
        if self.update_per_batch:
            ...
            kl_weight = (2**(self.steps_per_epoch-batch))/((2**(self.steps_per_epoch)-1))
            kl_weight = np.maximum(0.0, np.minimum(kl_weight, 1.0))
            self.kl_weight = kl_weight
